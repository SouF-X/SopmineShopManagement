...
public sealed class FamilleProduit : AuditableEntity
{
    public string Libelle { get; private set; } = string.Empty;

    private FamilleProduit() { }

    private FamilleProduit(Guid id, string libelle) : base(id)
    {
        Libelle = libelle;
    }

    public static Result<FamilleProduit> Create(Guid id, string libelle)
    {
        return new FamilleProduit(id, NormalizeText(libelle));
    }
